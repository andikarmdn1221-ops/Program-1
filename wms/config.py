...
DATABASE_READ_TIMEOUT_SECONDS = max(
    8, min(20, int(_SECRETS.get("DATABASE_READ_TIMEOUT_SECONDS", 12)))
)
...
RESTOCK_TARGET_MULTIPLIER = max(
    1, min(5, int(_SECRETS.get("RESTOCK_TARGET_MULTIPLIER", 2)))
)
