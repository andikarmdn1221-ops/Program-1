...
APP_VERSION = "8.4-account-security"
...
SESSION_REVALIDATE_SECONDS = max(30, int(_SECRETS.get("SESSION_REVALIDATE_SECONDS", 60)))
TELEGRAM_RETRY_ATTEMPTS = max(1, min(5, int(_SECRETS.get("TELEGRAM_RETRY_ATTEMPTS", 3))))
DATABASE_RETRY_ATTEMPTS = max(1, min(4, int(_SECRETS.get("DATABASE_RETRY_ATTEMPTS", 3))))
DATABASE_RETRY_BACKOFF_SECONDS = max(
    0.2, min(3.0, float(_SECRETS.get("DATABASE_RETRY_BACKOFF_SECONDS", 0.7)))
)
...
HEALTH_TIMEOUT_SECONDS = max(5, min(20, int(_SECRETS.get("HEALTH_TIMEOUT_SECONDS", 10))))
